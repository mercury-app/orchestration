...
class MercuriEdge:
    ...
    def add_source_dest_connect(self, source_dest_pair: Tuple[str, str]):
        ...
        if source_dest_pair[1] not in self._dest_node.input.keys():
            raise Exception("destination input is not an input of the destination node")

        self._source_dest_connect.add(source_dest_pair)
